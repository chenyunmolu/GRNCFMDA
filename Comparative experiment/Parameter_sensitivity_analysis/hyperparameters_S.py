...
增强特征矩阵的数量:S
'''
data = pd.read_excel("Parameter_sensitivity_analysis.xlsx", sheet_name='HMDAD_S', header=0)
S = data['S']
auc_values = data['AUC']
...
# 绘制 AUC 曲线
auc_line, = ax1.plot(S, auc_values, color='red', marker='p', lw=4, linestyle='dashed', label='AUC',
                     markersize=20)  # 红色，虚线
ax1.set_xlabel('S', fontsize='32')
ax1.set_xticks(S)
...
# 添加第二个 y 轴
ax2 = ax1.twinx()
aupr_line, = ax2.plot(S, aupr_values, color='#1D71AA', marker='*', lw=4, linestyle='dashed', label='AUPR',
...
plt.savefig('hyperparameters_S.png')
# plt.show()
