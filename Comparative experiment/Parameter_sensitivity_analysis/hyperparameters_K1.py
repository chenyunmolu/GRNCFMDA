...
随机传播的步数:K
'''
data = pd.read_excel("Parameter_sensitivity_analysis.xlsx", sheet_name='HMDAD_K1', header=0)
K = data['K']
auc_values = data['AUC']
...
# 绘制 AUC 曲线
auc_line, = ax1.plot(K, auc_values, color='red', marker='p', lw=4, linestyle='dashed', label='AUC',
                     markersize=20)  # 红色，虚线
...
# 添加第二个 y 轴
ax2 = ax1.twinx()
aupr_line, = ax2.plot(K, aupr_values, color='#1D71AA', marker='*', lw=4, linestyle='dashed', label='AUPR',
...
plt.savefig('hyperparameters_K1.png')
# plt.show()
